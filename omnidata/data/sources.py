...
class DataSource(AbstractCollection):
...
class AliasedDataSource(AbstractDataSource):
	...
	def has(self, name):
		return super().has(name) and (not isinstance(self._table[name], str) or self.has(self._table[name]))
